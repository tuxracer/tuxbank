...
- The toolbar **category filter** is **per category**: a toggle per category currently in use; each can be turned on/off independently, all shown by default. There is no **Uncategorized** toggle, since that is the absence of a choice rather than a category the user made; uncategorized and orphaned events always stay visible. The filter affects both which event chips display and the running balance (§4.7), so the visible events always sum to the balances shown beside them.
...
### 4.7 Account balance
- Each event is a transaction (deposit or withdrawal). Each day cell shows the **cumulative running balance**: starts at `0`, equals all deposits minus withdrawals up to and including that day, and carries continuously across months. Computed by the pure `src/lib/balance` engine (`computeRunningBalances`): a per-event carry-in for transactions before the visible window plus the windowed per-day net, accumulated forward (recurrence iteration is uncapped so long/infinite series sum correctly). Balances render cyan when ≥ 0 and magenta when negative, in every layout: compact day cells carry the same figure abbreviated (see Responsive / compact mode). The toolbar HUD shows the end-of-window balance. The balance honors the active category filter: hiding a category removes its amounts from every displayed balance, so the visible events always sum to the figures shown.
...
### Date-shift and series-shape helpers (`src/lib/recurrence`)

Five pure helpers support moving events to a different day and answering what a series looks like around a given occurrence. Each is exported and covered by tests in `src/lib/recurrence/tests.ts`.
...
- **`hasOccurrenceAfter(event, afterISO)`** reports whether the series has another occurrence strictly after `afterISO`. A rule with no `endsOn` always does (overrides are a finite list, so cancellations cannot exhaust an unbounded series); a bounded one is walked from the day after to `endsOn`, using `forEachOccurrence`'s bulk visitor so a long daily series costs O(overrides). Drives the scope dialog's `allowFollowing` (see §7).
...
- **This event** is dropped when an edit changes the recurrence rule, which a single-occurrence patch cannot carry (`allowThis`, decided by `isSameRecurrence`).
- **This and following** is dropped when nothing follows the occurrence and This event is on offer, since the split would then land on that one occurrence and repeat the choice above it (`allowFollowing`, decided by `hasOccurrenceAfter`). An edit that changes the rule keeps the option: the split re-anchors the series here, so it still carries the new schedule forward from what used to be the last occurrence.
- **All events** is dropped on the series' first occurrence while This and following is on offer, since the split already spans every occurrence from there and All events would repeat it (`allowAll`, decided in `App.tsx` by comparing the occurrence date to the event's anchor date). An edit that changes the rule on the first occurrence therefore has exactly one scope left (This event and All events are both dropped), so it applies as "this and following" without showing the dialog.
...
When `D` is the series' first occurrence, "this and following" spans the whole series and `CalendarContext` collapses it instead of splitting (a split would leave the truncated head with zero occurrences: a ghost row that never renders but syncs and exports forever). Edit rewrites the series row in place, keeping its id and dropping the override at `D` (superseded by the submitted input, matching the split's rule); delete removes the row; move slides the whole series via `shiftSeries`.
...
The console is the page's one bold element. It renders a static March 2026 built from the real `.cy-cell` / `.cy-chip` / `.cy-balance` markup, so it stays honest as the design system changes, and it carries a rail of month totals (deposits, withdrawals, month end). Its figures are never hand-written: `buildPreviewMonth` walks the transactions in `consts.ts` once and carries the running balance forward the way `src/lib/balance` does for real data, so the per-cell balances and the rail totals cannot drift from the events above them. The month is picked to show the product's point, not a best case: rent on the 1st overdraws the account for two days (magenta `.cy-balance-neg`) before the first paycheck lands. Below `sm`, where seven columns stop being readable, the grid is replaced by a **ledger tape**: the same month as a vertical list of date, chip, and running balance. Clicking Try Now sets a localStorage flag (`tuxbank:landing-dismissed`, see `src/lib/landingGate`) and swaps in the calendar; later visits with the flag boot straight into the app. One flow bypasses the landing page: a `#device-link=` URL, since a device-link sign-in must surface its TOTP prompt immediately. Signing out goes the other way: the storage wipe takes the flag with it, so the reload returns this browser to the landing page.
...
- **`scope === "following"` on a recurring event:** truncates the original series to end the day before `occurrence.date` via `truncateBefore`, then creates a new tail series anchored at `toDate` via `buildMovedFollowing`. From the series' first occurrence there is nothing to truncate away, so this case collapses into the `shiftSeries` slide above (one row, id kept, no ghost head).
...
- **Offline-first, but never a deploy behind:** fully functional with no network after the first visit, and current whenever there is one. A Workbox service worker (generated by `vite-plugin-pwa`, registered in `src/main.tsx`) precaches the entire build (JS, CSS, fonts, icons, `index.html`), so a cold load works with zero connectivity. Navigations go network-only (`runtimeCaching` in `vite.config.ts`) with a 3 second timeout, so an online visitor gets the current `index.html` and with it the current hashed bundle, on the load itself rather than one load later. When the network is unreachable or unresponsive the fallback is the precached `index.html` and nothing else, because it is the one copy always consistent with the assets the same worker precached. A runtime navigation cache used to sit in that chain and was removed: the worker auto-updates in place between navigations, so a cached navigation could name hashed bundles that later deploys had purged from both the precache and the CDN, and serving it on a failed reload produced a blank page while a working precached shell sat unused (`src/main.tsx` deletes the orphaned cache from existing installs). Everything else stays cache-first out of the precache; a new build's asset hashes simply miss and fall through to the network. Workbox's two cache-first defaults for navigations, `navigateFallback` and `directoryIndex`, are both cleared in `vite.config.ts`, because either one answers a navigation from the precache before the network-first route is reached. The worker still updates itself in the background (`registerType: "autoUpdate"`), but the register client's automatic page reload is suppressed with `onNeedReload`: the page is already running the newest build, so a reload would only discard whatever the user was in the middle of. Only precached files are served from cache; Supabase requests always go to the network.
- **A blank page says so:** the app is one non-split bundle, so a bundle that never arrives leaves a blank page with nothing running to explain it. `index.html` carries a boot notice that appears when `#root` is still empty five seconds in, telling the user to reload past the cache and that their saved events are untouched. The CSP allows no inline script, so the notice is CSS alone: `#root:empty ~ #boot-fallback` plus a delayed keyframe that reveals it, which keeps it off a load that is merely slow and retracts it by itself the moment React renders anything. Its colors and fonts are repeated from `globals.css` rather than read from its tokens, because the stylesheet is a separate request that usually goes missing alongside the bundle. Two invariants keep it working, neither of them enforced by a test: nothing may be pre-rendered into `<div id="root">`, and the top-level component must always render an element rather than `null`. Disabled JavaScript produces the same empty `#root` but calls for different advice, so a `<noscript>` in the head hides `#boot-fallback` and a second `<noscript>` in the body states that the app needs JavaScript and stays put until it is on. Both notices draw from the same `.boot-notice` styles, and only one of them can ever be visible.
- **Deployment headers** (`vercel.json`): `sw.js` and everything under `/assets/` are pinned to `max-age=0, must-revalidate`, so the worker that decides everything above is revalidated on every load. Vercel's default for the remaining static files is the same. The asset filenames are content-hashed, which would ordinarily justify a year of `immutable`, and that is deliberately not set: Vercel applies a `headers` rule by request path whatever status the response ends up carrying, so a 404 under `/assets/` is served with the asset cache header. A browser that fetches a new deployment's `index.html` during the seconds before that deployment's files answer everywhere gets a 404 for the hashed bundle the HTML references, and `immutable` pins that 404 for a year without revalidating it even on a normal reload, leaving a blank page that only a hard reload clears. Revalidating costs one conditional request per asset, and only on the loads the service worker does not already answer out of its precache.
- **Security headers** (`vercel.json`, all paths): a **Content-Security-Policy** plus `X-Content-Type-Options: nosniff`, `X-Frame-Options: DENY`, `Referrer-Policy: no-referrer`, and a `Permissions-Policy` denying the device APIs the app never uses. HSTS comes from Vercel. The CSP is what keeps an injected script from being the whole ballgame: the data key is cached in plaintext IndexedDB and the Supabase session in localStorage, so script injection defeats the E2EE outright, and everything the app loads is same-origin, which makes `default-src 'self'` cheap to hold. Two allowances are load-bearing and were found by loading the built app under the real header rather than by reading it: `script-src` needs **`'wasm-unsafe-eval'`** or libsodium's WebAssembly module refuses to instantiate and every crypto path dies at boot (`'wasm-unsafe-eval'` permits WASM compilation without permitting string eval), and `style-src` needs `'unsafe-inline'` for the style tags sonner and Radix inject at runtime. `connect-src` allows `https://*.supabase.co` for sync; analytics is same-origin under `/_vercel/`. Zod probes for the `Function` constructor to JIT-compile validators and logs one caught CSP eval violation per load as a result; it falls back to its interpreted path on its own. Verify any change to this header in a real browser (nothing in vitest covers CSP): serve `dist/` with the header applied, then load the app, save an event, and reach the sign-in form, which is the first thing that runs Argon2id.
...
- The data key is **cached at rest** in IndexedDB so a signed-in session stays
  ...
  cached key), clears `localStorage` and `sessionStorage`, and reloads onto the
  landing page (the wipe takes the landing-dismissed flag with it). It is
