...
- **Month console + rail:** the calendar sits inside a bordered console panel that follows the active theme, with `MonthRail` as its header strip: the visible month set in the display face beside a Deposits / Withdrawals / Month-end readout. `computeMonthTotals` derives the figures from the same per-day occurrence and balance maps the grid renders (respecting the category filter), so the rail can never disagree with the cells below it; withdrawals accumulate negative, and month end is the last in-month day's running balance. The structure mirrors the landing page's preview console, so in dark mode the app matches the landing's claim exactly; in light mode the same instrument renders in the light ink set.
