...
- **Deployment headers** (`vercel.json`): content-hashed files under `/assets/` are served `immutable` for a year, and `sw.js` is pinned to `max-age=0, must-revalidate` so the worker that decides everything above is itself revalidated on every load. Vercel's default for the remaining static files is already `max-age=0, must-revalidate`.
