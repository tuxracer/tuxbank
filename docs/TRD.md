...
- **Full-viewport layout:** `HUD status line` → `toolbar` → `month console` (the console flexes to consume all remaining height and holds the `month rail` → `weekday header` → `month grid`). On desktop the grid renders only the week-rows the visible month spans (4-6, via `inMonthWeekCount`), so day cells get more height; trailing weeks that fall entirely in the next month are dropped. Compact mode always renders the full 6 rows (see Responsive / compact mode).
- **Toolbar:** ‹ / month-year / › · **Today** · **category filter** · **+ New Event** (primary CTA).
- **Month console + rail:** the toolbar's calendar sits inside a `.cy-console` instrument (see §10): the dark ink set in both themes, with `MonthRail` as its header strip, the visible month set in the display face beside a Deposits / Withdrawals / Month-end readout. `computeMonthTotals` derives the figures from the same per-day occurrence and balance maps the grid renders (respecting the category filter), so the rail can never disagree with the cells below it; withdrawals accumulate negative, and month end is the last in-month day's running balance. This is the same console the landing page previews, so the app delivers the look the landing claims.
...
- Month console (`.cy-console`): pins the **dark** token set in both themes; used by both the landing preview and the app's real calendar (rail + weekday header + grid). The accents are data ink and only read as ink on a dark surface, so on the light theme the month renders as an instrument set on paper instead of a white box on grey. The values are copied from the `prefers-color-scheme: dark` block rather than invented, so the two must be kept in sync. `.cy-ink` shares the same token block but not the panel chrome: it re-pins the instrument ink on calendar satellites that render outside the console's DOM subtree (the drag-overlay chip, the day-events popover, the compact `DayPanel`).
...
- Both themes are selected automatically from `prefers-color-scheme`; there is **no in-app toggle and no persistence**. Pure CSS: no JS, no theme class. The theming applies to the page chrome (background, toolbar, dialogs, landing page); the month console and its satellites are exempt, pinning the dark ink set in both themes (see Month console above).
...
src/
  ...
  components/
    ...
    MonthRail/              # console header rail: month label + Deposits / Withdrawals / Month-end readout (computeMonthTotals)
