...
- Landing console (`.cy-console`): pins the **dark** token set in both themes. The accents are data ink and only read as ink on a dark surface, so on the light theme the month renders as an instrument set on paper instead of a white box on grey. The values are copied from the `prefers-color-scheme: dark` block rather than invented, so the two must be kept in sync.
