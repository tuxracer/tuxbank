...
- **Every control is one 32px box, and the primitive owns it**: `Button` (default size) and `Input` are both `h-8 px-2.5` over a `border-input` edge, and `--input` points at `--cy-line` so a control's border is the same line every panel draws. Height therefore comes from the component, never from a call-site `py-*`: a raw `<button className="cy-btn px-3 py-1.5">` computes to 30px next to a 32px sibling, which is how the toolbar ended up with three heights in one row. `.cy-btn` / `.cy-cta` supply the mono or display voice, never the box. A native `<select>` is the one control with no primitive to inherit from, so it states `h-8` explicitly. Never re-tint a control's border with a Tailwind `border-*` utility, because `.cy-*` is unlayered and wins: change the token instead.
...
- **Reach for the shadcn primitives**: Build interactive controls from `src/components/ui/` (`Button`, `Input`, `Textarea`, `Label`, `Popover`, …), including toolbar chrome and popover triggers, not from raw `<button>` / `<input>` elements. The primitive carries the shared box, focus treatment, and disabled and pressed states; hand-rolled elements drift from it one call site at a time. Layer the cyberpunk voice on top with `className="cy-btn …"` (see `SyncDialog`, `CalendarToolbar`), and use `asChild` on a Radix trigger so it renders a `Button`. The exception is a control no primitive covers: a native `<select>` stays native, because it opens the OS picker on mobile and works with react-hook-form's `register()`, where Radix's `Select` needs a `Controller`.
