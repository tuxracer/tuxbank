...
Two things are legitimately not settings and stay device-local: **caches** that only exist to make a device faster (the display-preferences first-paint mirror), and **device state** that is not a user choice about their data (`landingGate`'s "has this browser seen the landing page" flag). If a value would look wrong on another device, it is a cache; if the user picked it, it syncs.
