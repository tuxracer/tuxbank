...
This project's direction is already committed: a sober, cyberpunk-inspired data-ink aesthetic, flat panels and straight 1px borders with no glow, overlay-line, or gradient effects, set in Rajdhani (display) / Chakra Petch (UI) / JetBrains Mono (figures). Color is reserved for meaning, not decoration: cyan is the only interface accent, green and magenta mark deposits and withdrawals, yellow marks today, and orange is category-only. Read [TRD §10](docs/TRD.md) before touching visuals and extend that language instead of inventing a new one. The rules below apply when building a genuinely new surface (promo page, diagram, docs site) that the TRD does not already cover.
