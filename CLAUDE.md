...
- **Icon rasters are hand-rendered and nothing regenerates them**: `public/icon.svg` and `public/icon-maskable.svg` are the sources; `icon-192.png`, `icon-512.png`, `icon-maskable-512.png`, `apple-touch-icon.png`, and `favicon.ico` were rendered from them with a one-off `sharp` install that was uninstalled afterward. No build step or script redoes it, so editing an SVG without re-rendering the whole set ships a mark that only matches on the browser tab. The maskable variant scales the grid to 72% so the corner cells clear the circular safe zone. `favicon.ico` is deliberately referenced by neither `index.html` nor the manifest (modern browsers take the SVG); it answers only legacy clients and bots that fetch `/favicon.ico` blindly, and `ico` is in the workbox glob so it precaches with the rest.
