...
- **`src/components/`** — UI: `MonthGrid`, `DayCell`, `EventChip`, `EventDialog`, `CategoryCombobox`, `ManageCategoriesDialog`, `RecurrenceScopeDialog`, `CalendarToolbar` (month/year nav), `DataDialog` (JSON backup export/import), `SyncDialog` (optional account sync: create / sign-in / TOTP / recovery-key / change-password / device-link QR), `AboutDialog`, `LandingPage` (first-visit entry screen with the Try Now CTA), `DayEventsPopover`, `DayPanel` (compact-mode selected-day detail), `DraggableEventChip` (drag events between days), `StorageUnavailableBanner`, `SyncAttentionBadge`, `CyberFrame`, `CyControlFrame`, … · shadcn primitives in `src/components/ui/`.
...
This project's direction is already committed: "Night City" cyberpunk, a cyan/magenta synthwave palette, Rajdhani (display) / Chakra Petch (UI) / Share Tech Mono (mono). Read [TRD §10](docs/TRD.md) before touching visuals and extend that language instead of inventing a new one. The rules below apply when building a genuinely new surface (promo page, diagram, docs site) that the TRD does not already cover.
