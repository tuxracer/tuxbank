...
- **Scroll containers clip focus rings**: `overflow-y-auto` clips the x axis too (the computed `overflow-x` becomes `auto`), so anything a child paints outside its border box is sliced off at the left and right edges. shadcn's `focus-visible:ring-3` paints 3px outside the box, so a full-width input inside a scrolling wrapper loses its ring on both sides. Give any scroll container that holds focusable controls `-mx-1 px-1`, which buys 4px of inline room without shifting the content. The four dialog bodies (`EventDialog`, `SyncDialog`, `DataDialog`, `ManageCategoriesDialog`) carry it; keep it when copying that pattern. The same applies to any other outside-the-box paint (rings, `box-shadow`, `outline`) inside a clipping ancestor.
