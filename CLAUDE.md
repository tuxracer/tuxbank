...
- **`src/components/`** — UI: `MonthGrid`, `MonthRail` (console header: month label + deposits/withdrawals/month-end readout), `DayCell`, `EventChip`, `EventDialog`, `CategoryCombobox`, `ManageCategoriesDialog`, `RecurrenceScopeDialog`, `CalendarToolbar` (month/year nav), `DataDialog` (JSON backup export/import), `SyncDialog` (optional account sync: create / sign-in / TOTP / recovery-key / change-password / device-link QR), `LandingPage` (first-visit entry screen with the Try Now CTA; also the only place the app links its source repository and license), `DayEventsPopover`, `DayPanel` (compact-mode selected-day detail), `DraggableEventChip` (drag events between days), `StorageUnavailableBanner`, `SyncAttentionBadge`, … · shadcn primitives in `src/components/ui/`.
