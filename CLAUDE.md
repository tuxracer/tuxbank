...
- **Form controls share one box**: `Input` (`src/components/ui/input.tsx`) defines the field geometry, `h-8 px-2.5 text-sm` over a `border-input` edge, and the `--input` token points at `--cy-line` so a control's border is the same line every panel draws. Anything standing in for a text field (a raw `<select>`, a popover trigger like `CategoryCombobox`) carries those same utilities instead of inventing its own padding; `.cy-btn` on such a control supplies only the mono voice, never the box. Never re-tint a control's border with a Tailwind `border-*` utility, because `.cy-btn` is unlayered and wins: change the token. Fields sit side by side in `flex` rows (Amount / Type, Repeat / Every / Until), where a 6px height difference or a 2px inset difference is immediately visible.
