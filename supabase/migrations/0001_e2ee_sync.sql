-- E2EE sync: tables, indexes, and RLS for optional account sync.
--
-- The primary key on events/categories is composite (user_id, id): ids are
-- client-generated UUIDs that travel inside JSON backups, so the same id can
-- legitimately exist under two accounts. PostgREST resolves upserts on the
-- primary key, so a composite key confines ON CONFLICT to the caller's own
-- rows; a global id key made a backup imported under a second account fail
-- sync with RLS error 42501 on the ON CONFLICT UPDATE path.
